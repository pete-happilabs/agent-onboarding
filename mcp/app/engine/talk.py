...
async def talk(event: Dict[str, Any]) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"TALK - MCP Response: {len(mcp_response)} chars, Metrics: {metrics.to_dict()}")
        ...
        return _build_response(
            ...
            destination_entity_id=entity_id,
            session_id=session_id,
            message_text="Sorry, I encountered an error processing your request. Please try again.",
            event_hint="error",
        ), metrics.to_dict()
