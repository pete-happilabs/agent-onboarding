"""
Unit tests for dostEvent parsing - MCP agent
"""
import pytest
import sys
...
class TestDostEventParser:
    ...
    def test_parse_valid_dostevent(self, base_dostevent):
        ...
        assert base_dostevent["destinationEntityId"] == "agent.test"
...
# === NEW: Production tests using REAL protocol functions ===
...
class TestRealProtocolFunctions:
    ...
    def test_create_dost_event_minimal(self):
        ...
        assert event["sourceEntityId"] == "agent.test"
    ...
    def test_create_dost_event_full(self):
        ...
        event = create_dost_event(
            source_entity_id="agent.mcp.test",
            destination_entity_id="hum.user.123",
            session_id="session-456",
            event_hint="response",
            is_ai_generated=True,
            message=create_dost_message(text="Here are your results")
        )
        
        assert event["version"] == DOST_SPEC_VERSION
        assert event["sourceEntityId"] == "agent.mcp.test"
        ...
        assert event["message"]["text"]["data"] == "Here are your results"
