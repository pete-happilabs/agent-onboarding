# ============================================================================
# FILE: shared/dost/protocol.py — Canonical DOST protocol for all templates
# ============================================================================
"""
...
_ENTITY_ID_PATTERN = re.compile(r"^[\w.\-:@]+$")
...
def create_dost_event(
    ...
    if not source_entity_id or len(source_entity_id) > MAX_ENTITY_ID_LENGTH:
        raise ValueError(f"sourceEntityId must be 1-{MAX_ENTITY_ID_LENGTH} characters")
    if not _ENTITY_ID_PATTERN.match(source_entity_id):
        raise ValueError("sourceEntityId contains invalid characters (allowed: alphanumeric, ., -, _, :, @)")
...
def create_dost_message(
    ...
    if text is not None:
        if len(text) > MAX_TEXT_LENGTH:
            raise ValueError(f"Message text exceeds maximum length of {MAX_TEXT_LENGTH}")
...
def create_dost_pricing(
    ...
    if not isinstance(value, (int, float)) or math.isnan(value) or math.isinf(value):
        raise ValueError("pricing value must be a finite number")
    if value < 0:
        raise ValueError("pricing value must be non-negative")
...
def create_dost_action(
    ...
    if url is not None:
        if url.lower().startswith(("javascript:", "data:", "vbscript:")):
            raise ValueError("Action URL must not use javascript:, data:, or vbscript: schemes")
        action["url"] = url
...
# Validation
# =============================================================================
def validate_dost_event(event: Dict[str, Any]) -> List[str]:
    """
    Validate a dostEvent dict against the spec. Returns a list of errors (empty = valid).

    Checks:
    - Required fields present and typed correctly
    - sourceEntityId format
    - At least one of message or categories present
    """
    errors: List[str] = []

    if not isinstance(event, dict):
        return ["dostEvent must be a dict"]

    # Required fields
    for field in ("version", "timestamp", "eventId", "sessionId", "sourceEntityId"):
    ...
    # sourceEntityId format
    sid = event.get("sourceEntityId")
    if isinstance(sid, str):
        if len(sid) > MAX_ENTITY_ID_LENGTH:
            errors.append(f"sourceEntityId exceeds {MAX_ENTITY_ID_LENGTH} chars")
        elif not _ENTITY_ID_PATTERN.match(sid):
            errors.append("sourceEntityId contains invalid characters")

    # isAiGenerated
    if "isAiGenerated" in event and not isinstance(event["isAiGenerated"], bool):
        errors.append("isAiGenerated must be a boolean")

    # At least message or categories
    if event.get("message") is None and event.get("categories") is None:
        errors.append("dostEvent must contain at least message or categories")
...
def extract_query_text(event: Dict[str, Any]) -> str:
    ...
    try:
        ...
        logger.debug("extract_query_text failed: %s", e)
        return ""
...
def extract_objects_from_categories(event: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.debug("extract_objects_from_categories failed: %s", e)
        return []
