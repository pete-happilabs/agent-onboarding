#!/usr/bin/env python3
# ============================================================================
# TheMealDB Recipe Playground
# ============================================================================
"""
Test the Custom REST API Agent with the FREE TheMealDB API.
NO API KEY REQUIRED - works immediately!

Usage:
    python3 mealdb-playground.py
    python3 mealdb-playground.py -v
    python3 mealdb-playground.py -m "Search for pasta recipes"
"""
import argparse
import asyncio
import json
...
os.environ["CUSTOM_CONFIG_PATH"] = str(Path(__file__).parent / "configs" / "mealdb.yaml")
os.environ["AGENT_ENTITY_ID"] = "agent.custom.mealdb"
os.environ["AGENT_NAME"] = "Recipe Assistant"
...
async def playground(verbose: bool = False, full_json: bool = False):
    ...
    print("\n" + "="*60)
    print("  TheMealDB Recipe Playground")
    print("="*60)
...
def main():
    parser = argparse.ArgumentParser(description="TheMealDB Recipe Playground")
    parser.add_argument(
        "--config", "-c",
        default=None,
        help="Path to config file (default: configs/mealdb.yaml)"
    )
    parser.add_argument(
        "--verbose", "-v",
        action="store_true",
        help="Show verbose agent activity (tool calls, args, results)"
    )
    parser.add_argument(
        "--full", "-f",
        action="store_true",
        help="Show full JSON payloads by default"
    )
    parser.add_argument(
        "--message", "-m",
        default=None,
    ...
    if args.config:
        os.environ["CUSTOM_CONFIG_PATH"] = args.config
